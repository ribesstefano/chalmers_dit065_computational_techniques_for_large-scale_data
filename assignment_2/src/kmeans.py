...
def kmeansSerial(k, data, nr_iter=100, p=1):
    ...
    print(f'Total time:                    {total_time:3.2f} [s]')
    print(f'Total serial time:             {serial_time:3.2f} [s] (f_seq:{serial_time / total_time:.2f})')
    print(f'Total nearestCentroid time:    {nearestCentroid_t:3.2f} [s] (f_par:{nearestCentroid_t / total_time:.2f})')
    print(f'Total recomputeCentroids time: {recomputeCentroids_t:3.2f} [s] (f_par:{recomputeCentroids_t / total_time:.2f})')
    print(f'Avg. nearestCentroid time:     {nearestCentroid_t / nr_iter:3.2f} [s]')
    print(f'Avg. recomputeCentroids time:  {recomputeCentroids_t / nr_iter:3.2f} [s]')

    print('=' * 80)
    T_seq = total_time - nearestCentroid_t
    T_par = nearestCentroid_t
    print(f'Amdhal Sp(nearestCentroid, {p}) = {amdhals_law(T_seq, T_par, p):.3f}')

    T_seq = total_time - recomputeCentroids_t
    T_par = recomputeCentroids_t
    print(f'Amdhal Sp(recomputeCentroids, {p}) = {amdhals_law(T_seq, T_par, p):.3f}')
    print('=' * 80)
...
def kmeans(k, data, nr_iter=100):
    ...
    for j in range(nr_iter):
        ...
        slices = list(split(list(range(N)), args.workers))
